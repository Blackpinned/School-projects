...
public class MandjeController {
	...
	private List<Pizza> maakPizzasVanPizzaIds(List<Long> pizzaIds) {
		...
		final List<Pizza> pizzas = new ArrayList<>(pizzaIds.size());
		for (final Long id : pizzaIds) {
	...
	ModelAndView toonMandje() {
		...
		return new ModelAndView(VIEW).addObject(new MandjeForm()).addObject("allePizzas", pizzaService.findAll())
