...
class IndexController {
	...
	ModelAndView index(
		...
		String boodschap;
		final int uur = LocalTime.now().getHour();
		...
		final Cookie cookie = new Cookie("laatstBezocht", LocalDateTime.now().toString());
		cookie.setMaxAge(31_536_000);
		
		response.addCookie(cookie);
		
		final ModelAndView modelAndView = new ModelAndView("index", "boodschap", boodschap).addObject(
				"zaakvoerder",
