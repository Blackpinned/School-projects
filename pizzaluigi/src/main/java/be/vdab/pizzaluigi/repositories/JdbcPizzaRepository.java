...
public class JdbcPizzaRepository implements PizzaRepository {
	...
	private static final String DELTE_PIZZA = "delete from pizzas where id=:id";
	private static final String UPDATE_PIZZA = "update pizzas set naam = :naam, prijs = :prijs, pikant = :pikant where id = :id";
	private final SimpleJdbcInsert insert;
	private final RowMapper<Pizza> pizzaRowMapper = (resultSet, rowNum) -> new Pizza(resultSet.getLong("id"),
			resultSet.getString("naam"), resultSet.getBigDecimal("prijs"), resultSet.getBoolean("pikant"));
	...
	public void delete(long id) {
		template.update(DELTE_PIZZA, Collections.singletonMap("id", id));
	}
